    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                print("\nfetch_one_query: row_data => ", row_data,"\t", type(row_data),"\n")

                if row_data:
                    column_names = [desc[0] for desc in cur.description]
                    row_dict = dict(zip(column_names, row_data))
                    print("\nfetch_one_query: row_dict => ", row_dict,"\t", type(row_dict),"\n")
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                # Create a dictionary with column names as keys and lists of column values
...
async def fetching_account_id_by_vendor_id(db_pool: aiomysql.pool.Pool, account_id: str) -> Dict[str,str]:
